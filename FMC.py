...
def ipr(l):
    ...
    def unpack_constants(term):
        if isinstance(term, Variable):
            if term.value in FUNCTIONS:
                new=FUNCTIONS[term.value]
            return new.compose(unpack_constants(term.next))
        elif isinstance(term, Application):
            return Application(term.location, unpack_constants(term.function),unpack_constants(term.argument))
        elif isinstance(term, Abstraction):
            return Abstraction(term.location, term.value, unpack_constants(term.body))
